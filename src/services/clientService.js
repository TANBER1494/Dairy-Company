...
async createClient(data) {
    ...
    if (existingClient) { 
      throw new AppError('كود العميل مسجل بالفعل', 400);
    }
  ...
  async getAllClients(query = {}) {
    return await Client.find(query).sort({ code: 1 }).lean();
  }
  ...
  async updateClient(id, data) {
    if (data.code) {
      ...
      if (existingClient) {
        throw new AppError('كود العميل مستخدم لعميل آخر', 400);
      }
    }
  ...
  async getClientStatement(searchKey) {
    ...
    if (!client) {
      throw new AppError('العميل غير موجود بهذا الكود أو الاسم', 404);
    }
